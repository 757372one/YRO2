/**    
 * https://t.me/YieldRocketTG
 * Tokenomics:
 * - 100 billion total supply.
 * - 1.5 billion max buy/hold/sell (1.5% of total supply).
 * - 400 Million max buy during anti-whale launch window.
 * - 10% YRO rewards 100% of which is rewarded to holders.
 * - 8% liquidity tax
 * - 2% development fund
 * - Anti-dump sell tax increase factor X 1.75 
//
...
contract YieldRocket is ERC20, Ownable {
    ...
    address public liquidityAddress = 0xF732BF47d551fB878E5F792198798531a32faCA0;
    address public _devAddress = 0xF732BF47d551fB878E5F792198798531a32faCA0;
    ...
    uint256 public devFund = 2;
    uint256 public totalFees = rewardsFee.add(liquidityFee).add(devFund);
    ...
    function setRewardsFee(uint256 value) external onlyOwner{
        rewardsFee = value;
        totalFees = rewardsFee.add(liquidityFee).add(devFund);
    }
    function setLiquiditFee(uint256 value) external onlyOwner{
        liquidityFee = value;
        totalFees = rewardsFee.add(liquidityFee).add(devFund);
    }
    function setDevFund(uint256 value) external onlyOwner{
        devFund = value;
        totalFees = rewardsFee.add(liquidityFee).add(devFund);
    }
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "YieldRocket: The dividend tracker already has that address");
        YieldRocketDividendTracker newDividendTracker = YieldRocketDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "YieldRocket: The new dividend tracker must be owned by the YieldRocket token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "YieldRocket: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "YieldRocket: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "YieldRocket: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "YieldRocket: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityAddress(address newLiquidityAddress) public onlyOwner {
        require(newLiquidityAddress != liquidityAddress, "YieldRocket: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "YieldRocket: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "YieldRocket: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(
            ...
            swapping = true;
            uint256 developmentFund = contractTokenBalance.mul(devFund).div(totalFees);
            swapAndSendToFee(developmentFund);
...
contract YieldRocketDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("YieldRocket_Dividend_Tracker", "YieldRocket_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }
    function _transfer(address, address, uint256) internal override {
        require(false, "YieldRocket_Dividend_Tracker: No transfers allowed");
    }
    function withdrawDividend() public override {
        require(false, "YieldRocket_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main YieldRocket contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "YieldRocket_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "YieldRocket_Dividend_Tracker: Cannot update claimWait to same value");
